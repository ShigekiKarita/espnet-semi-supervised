...
for tts_weight in 0.01 ; do
    for lr in 1e-5; do
        bs=24
        for loss in mmd kl; do
            opt=" --model_asr ${asr} --model_tts ${tts} --inter_domain_loss ${loss} --use_mmd_ae True "
            exp_prefix="1018_ae_${loss}_run6_retrain_bs24_lr${lr}_asr${asr_weight}_${etype}/sbatch"
            for mmd in 0.1 0.01 ; do
                ...
                sleep 2
                # # tts + s2s + t2t
                # ./run6.sh --stage 5 --ngpu ${ngpu} --etype ${etype} --lr ${lr} --batchsize ${bs} --mmd_weight ${mmd} --exp_prefix ${exp_prefix} --asr_weight 0.0 --tts_weight ${tts_weight} --s2s_weight ${ae_weight} --t2t_weight ${ae_weight} ${opt} &
                # sleep 2
            done
