...
lmexpdir=exp/train_large_char_rnnlm_2layer_bs256
mkdir -p ${lmexpdir}
if [ ${stage} -le 4 ]; then
    echo "stage 4: LM Preparation"
    lmdatadir=data/local/lm_train_large
    if [ ! -e ${lmdatadir}/valid.txt ]; then
        mkdir -p ${lmdatadir}
        cat data/train_clean_100_asr/text data/train_other_500_asr/text > ${lmdatadir}/text
        text2token.py -s 1 -n 1 ${lmdatadir}/text | cut -f 2- -d" " | perl -pe 's/\n/ <eos> /g' \
                                                                           > ${lmdatadir}/train.txt
        text2token.py -s 1 -n 1 data/${train_dev}_asr/text | cut -f 2- -d" " | perl -pe 's/\n/ <eos> /g' \
                                                                                    > ${lmdatadir}/valid.txt
        text2token.py -s 1 -n 1 data/test_clean_asr/text | cut -f 2- -d" " | perl -pe 's/\n/ <eos> /g' \
                                                                                  > ${lmdatadir}/test.txt
    fi
